...
import com.pplove.bip.scheduler.oozie.WorkflowSubmitter;
...
import static com.pplove.bip.util.DateUtil.*;
import java.util.*;
...
//    @Scheduled(fixedRate = 5000)
    ...
    public boolean  isLatestCalcComplete(SchedulableReport report, int date) {
        ...
        if (f == Frequency.DAILY) {
            //天报表判断前一天状态
            LocalDate dt = LocalDate.parse(String.valueOf(date), DateUtil.DAILY_FORMATTER);
            int yesterday = Integer.valueOf(dt.minusDays(1).format(DateUtil.DAILY_FORMATTER));
            ...
            if (date % 100 == 0) {
                ...
                LocalDateTime dateTime = LocalDateTime.parse(String.valueOf(date), DateUtil.HOURLY_FORMATTER);
                int last = Integer.valueOf(dateTime.minusHours(1).format(DateUtil.HOURLY_FORMATTER));
    ...
    public boolean  isCalcComplete(SchedulableReport dependency, int date, Frequency frequency) {
        ...
        switch (frequency) {
            ...
            case WEEKLY:
                ...
                LocalDate dt = LocalDate.parse(String.valueOf(date), DateUtil.DAILY_FORMATTER);
                for(int i = 0; i < 7; ++i) {
                    int day = Integer.valueOf(dt.plusDays(i).format(DateUtil.DAILY_FORMATTER));
            ...
            case MONTHLY:
                ...
                assert f == Frequency.DAILY;
                LocalDate m = LocalDate.parse(String.valueOf(date*100+1), DateUtil.DAILY_FORMATTER);
                LocalDate nextMonth =  m.plusMonths(1);
                while (m.isBefore(nextMonth)){
                    int day = Integer.valueOf(m.format(DateUtil.DAILY_FORMATTER));
                    if (isCalcComplete(dependency, day) == false) {
                        return false;
                    }
                    m = m.plusDays(1);
